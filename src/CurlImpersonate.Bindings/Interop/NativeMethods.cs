...
internal static partial class NativeMethods
{
    ...
    // All exports use the C calling convention (Cdecl). Forcing it here keeps the
    // source-generated [LibraryImport] stubs consistent with the native exports and
    // the [DllImport(Cdecl)] streaming entry below — and avoids a StdCall/Cdecl
    // mismatch should this ever be built/run as x86.
    ...
    // status_code is int64_t in the C ABI (NOT C `long`, which is 32-bit on Windows),
    // so the managed `long` is an exact match on every platform.
